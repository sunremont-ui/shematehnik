...
- `genLvglProject()`, the UI Designer state / `.ucp` migration wrapper, minimal clicked/value_changed event callback stubs, minimal `screen_load` actions, minimal bgColor/radius style tokens, `Image.assetId` placeholders, the `UiProjectDesign.assets` project manifest (id/src declarations + missing-asset report), minimal `Panel.layout` flex output and same-screen `Panel` child parents are in place; richer action graphs/styles, a binary/file asset pipeline, nested/responsive layouts and LVGL v9 mode are still pending.
