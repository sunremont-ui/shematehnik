...
// ============================================================
import type {
  UiW, UiProjectDesign, UiScreenDesign, PacketField, FsmDesign,
...
function emitImageAssetDecls(out: string[], widgets: UiW[]) {
  ...
  for (const w of widgets) {
    ...
    if (!asset || emitted.has(asset)) continue;
    emitted.add(asset);
    out.push(`LV_IMG_DECLARE(${asset});`);
  }
  if (emitted.size) out.push("");
}
...
export function genLvglProject(project: LvglProjectDesign): LvglOut {
  ...
  c.push(`#include "ui.h"`, "");
  emitImageAssetDecls(c, screens.flatMap((screen) => screen.widgets));
