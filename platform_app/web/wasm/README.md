...
`../public/wasm/`. Пересобрать после изменения `*.cpp`:
...
Результат — `../public/wasm/ucp_core.js` + `ucp_core.wasm` (ES-модуль,
...
Сгенерированные артефакты (`public/wasm/`) в `.gitignore` — собираются
