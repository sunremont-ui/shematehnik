...
- Widget visibility/opacity: a widget can store `hidden` (-> `lv_obj_add_flag(obj, LV_OBJ_FLAG_HIDDEN)`) and `opa` 0-254 (-> `lv_obj_set_style_opa(obj, n, LV_PART_MAIN | LV_STATE_DEFAULT)`); both emit only when set.
- Bundle export: the LVGL Export view can download `ui.c` + `ui.h` + `README.txt` (`genLvglReadme`) as a single `.zip` built by the pure `zipStore`/`crc32` in `image`-style `src/zip.ts` (STORED, no compression, no external dependency).
