...
| L3 Golden tests | [x] baseline + events/actions + styles + assets + manifest + layout/align/parents | Exact fixture для текущих `ui.c/ui.h` в `codegen.test.ts`; event, screen-load action, style, image asset, asset-manifest src/missing, inline RGB565/RGB565A8 descriptor, widget hidden/opacity, extended style tokens (text/border/pad/font), Panel flex layout, flex main/cross/track align, per-child flex grow and Panel child-parent checks added |
