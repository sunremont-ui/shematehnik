...
## 🔜 Фаза 7 — полный fab-комплект

1. **BOM-экспорт CSV** — группировка по value/типу.
2. **B.Cu Gerber + Excellon drill** — полный комплект файлов для производства.
3. **Net-метки / power-символы** (GND/VCC) — связность без проводов; поднимает точность импорта `.kicad_sch`.
4. **Поворот компонентов** (R) + junctions / наименование цепей.
