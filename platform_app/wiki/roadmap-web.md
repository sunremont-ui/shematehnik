...
| L1 Compatibility | [x] verified v8→v9 deltas | Матрица LVGL v8/v9 сверена с офиц. v9.0 CHANGELOG (слайс 19); реализован `mode: "v8"\|"v9"` dialect (слайс 22). v8 baseline байт-идентичен; SquareLine claims по-прежнему отложены до fixtures |
| L2 Model | [x] multi-screen + events/actions + styles + assets + manifest + layout/align/parents + v9 dialect | `UiProjectDesign`/`UiScreenDesign`, `uiProject`, `.ucp` migration wrapper, `genLvglProject()` with `mode: "v8"\|"v9"` dialect, minimal `UiEvent`, `screen_load` action, `UiStyle` (bg/radius/text/border/pad/Montserrat font/pressed), `Image.assetId`, `UiProjectDesign.assets` manifest (id/src + inline RGB565/RGB565A8 pixels), per-widget `hidden`/`opa`, `Panel.layout` (flow/gap + main/cross/track align), non-Panel `parentId` and per-child `flexGrow` metadata ready; nested/responsive hierarchy pending |
| L3 Golden tests | [x] baseline + events/actions + styles + assets + manifest + layout/align/parents | Exact fixture для текущих `ui.c/ui.h` в `codegen.test.ts`; event, screen-load action, style, image asset, asset-manifest src/missing, inline RGB565/RGB565A8 descriptor, widget hidden/opacity, extended style tokens (text/border/pad/font/pressed), v8/v9 dialect renames, Panel flex layout, flex main/cross/track align, per-child flex grow and Panel child-parent checks added |
| L4 Slice | [x] state + events/actions + styles + assets + manifest + layout/align/parents + bundle/v9 | Multi-screen generator/UI state plus clicked/value_changed callback stubs, screen-load controls/export, bgColor/radius + text/border/padding/font + pressed-fill controls, Image asset id export, project asset-manifest editor with image import (RGB565/alpha), widget Hidden/Opacity controls, Panel layout + main/cross/track align controls/export, Parent panel + per-child Grow controls/export, `Download .zip` bundle and a `Target: v8/v9` toggle |
| L5 Promotion | [x] current slices | Подтверждённые single/multi-screen, persistence, events/actions, full style tokens (+font/pressed), asset manifest + RGB565/RGB565A8 pipeline, flex layout/align/grow, parents, `.zip` bundle and v8/v9 dialect решения перенесены в `modules/codegen.md`, `modules/web_frontend.md`, `integration/squareline.md`, `compatibility-matrix.md`, roadmap и `log.md` |
